...
describe("costElectricalKM Test", () => {
    it("Calcular el costo usando consumption y precio de energia", () => {
...
describe("Resultados de ejemplo (pass / fail / skipped / cancelled)", () => {
    ...
    test("prueba intencionalmente fallida (fail)", () => {
        // Prueba fail
    ...
    ac.abort() 
    test("prueba cancelada (cancelled)", { signal: ac.signal }, () => {
        assert.fail("Esta línea no debería ejecutarse")
    })
})

describe("Funciones async: árboles e H2 (youngTree, oldTree, energyH2*, hydrogenMass, litersRequired)", () => {
    it("youngTree y oldTree devuelven NaN (implementación actual)", async () => {
    ...
    it("energyH2Cylinders, energyH2LowPresure, energyConsumed, hydrogenMass y litersRequired devuelven NaN (implementación actual)", async () => {
